...
// NUCLEAR OPTION: FORCE GOLD SHIMMER VISIBILITY
document.addEventListener('DOMContentLoaded', function() {
    // Fallback shimmer enforcement - runs after 2 seconds to ensure CSS has loaded
    setTimeout(function() {
        // Target both headers that should have gold shimmer
        const goldHeaders = [
        ...
        goldHeaders.forEach(header => {
            if (header) {
                // Add ultra-visible gold styling as backup
                header.style.background = 'linear-gradient(45deg, #ffd700 0%, #ffffff 15%, #fbbf24 30%, #ffffff 45%, #f59e0b 60%, #ffffff 75%, #ffd700 90%, #ffffff 100%)';
                header.style.backgroundSize = '400% 100%';
                header.style.webkitBackgroundClip = 'text';
                header.style.webkitTextFillColor = 'transparent';
                header.style.backgroundClip = 'text';
                header.style.animation = 'megaShimmer 1.5s linear infinite';
                header.style.filter = 'drop-shadow(0 0 20px rgba(255, 215, 0, 0.8)) drop-shadow(0 0 40px rgba(251, 191, 36, 0.6))';
                header.style.position = 'relative';
                header.style.zIndex = '10';
            }
        });
        
        // Extra safety: log for debugging (remove in production)
        console.log('🌟 Nuclear shimmer fallback applied to headers');
    }, 2000);
});

// EMERGENCY VISIBILITY TEST - Add this temporarily to see if ANY animation works
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        const testHeaders = document.querySelectorAll('.services h2, .contact h2');
        testHeaders.forEach(header => {
            if (header) {
                // Add pulsing border to make it impossible to miss if shimmer fails
                header.style.border = '3px solid rgba(255, 215, 0, 0.8)';
                header.style.borderRadius = '10px';
                header.style.padding = '10px 20px';
                header.style.boxShadow = '0 0 30px rgba(255, 215, 0, 0.6), inset 0 0 20px rgba(251, 191, 36, 0.3)';
                ...
                if (!document.getElementById('emergency-pulse-styles')) {
                    const style = document.createElement('style');
                    style.id = 'emergency-pulse-styles';
                    style.textContent = pulseKeyframes;
                    document.head.appendChild(style);
                }
                
                header.style.animation = 'megaShimmer 1.5s linear infinite, emergencyPulse 2s ease-in-out infinite';
            }
        });
        
        console.log('🚨 EMERGENCY VISIBILITY MODE ACTIVATED - Headers should be impossible to miss now!');
    }, 1000);
});
