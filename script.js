...
// Custom Success Message Function for Form Submissions
function showCustomSuccessMessage() {
    // Create custom popup element if it doesn't exist
    let customPopup = document.getElementById('customSuccessPopup');
    if (!customPopup) {
        customPopup = document.createElement('div');
        customPopup.id = 'customSuccessPopup';
        customPopup.innerHTML = `
            <div class="premium-popup-content">
                <div class="popup-header">
                    ...
                    <h2 class="popup-title">Successfully Submitted!</h2>
                    <button class="popup-close-btn" onclick="hideCustomSuccessMessage()">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
    ...
    // Detect mobile to adjust positioning
    const isMobile = window.innerWidth <= 768;
    
    if (isMobile) {
        // Mobile positioning - use fixed positioning at top of screen
        ...
        // Desktop positioning - center on screen
        customPopup.style.cssText = `
            display: block; 
            position: fixed; 
            top: 50%; 
    ...
    // Auto-hide after 6 seconds to give users time to read the message
    setTimeout(() => {
    ...
    // Add click outside to close
    setTimeout(() => {
        document.addEventListener('click', hideCustomSuccessMessageOnClickOutside, true);
    }, 100);
}
...
function hideCustomSuccessMessage() {
    ...
    if (popup) {
        popup.style.display = 'none';
        document.removeEventListener('click', hideCustomSuccessMessageOnClickOutside, true);
    }
}

let hideCustomSuccessMessageOnClickOutside = function(e) {
    const popup = document.getElementById('customSuccessPopup');
    const popupContent = popup?.querySelector('.premium-popup-content');
